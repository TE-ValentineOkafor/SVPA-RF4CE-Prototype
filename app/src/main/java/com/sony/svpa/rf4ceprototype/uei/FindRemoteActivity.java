...
public class FindRemoteActivity extends RemoteBaseActivity {
	...
	private void findRemote() {
		try {
			...
			Log.d(MainActivity.LOGTAG, "Start Find Remote result = " + result);
		} catch(Exception ex) {
	...
	private void cancelFindRemote() {
		try {
			...
			Log.d(MainActivity.LOGTAG, "Cancel FindRemote result = " + result);
		} catch(Exception ex) {
			Log.e(MainActivity.LOGTAG, ex.toString());
		}
	}
	...
	private IFindRemoteCallback _findRemoteCallback = new IFindRemoteCallback.Stub() {
		...
		public void onStatusUpdated(int remoteId, int resultCode) {
			try {
				Log.d(MainActivity.LOGTAG, "Find Remote status: " + resultCode + " - " +

...
public class FirmwareUpdateActivity extends RemoteBaseActivity {
				...
				try {
					...
					if (inputFile.length() >= 0) {
						...
						if(_setup != null ){
							...
							Log.d(MainActivity.LOGTAG, "updateFirmware status: " + ResultCode.getString(status));
					...
					Log.d(MainActivity.LOGTAG, "updateFirmware exception: " + ex.toString());
				}
			}
		});
		...
		if(this._updatingFirmware) {
			try {
				if(_setup != null ){
					...
					Log.d(MainActivity.LOGTAG, "cancelUpdateFirmware status: " + ResultCode.getString(status));
				}
			} catch (Exception ex) {
				Log.d(MainActivity.LOGTAG, "cancelUpdateFirmware exception: " + ex.toString());
			}
		}
	}
	...
	private void updateProgressBar(final int progress) {
		Log.d(MainActivity.LOGTAG, "updateProgressBar: " + progress);
		...
		if(resultCode == RESULT_OK) {
			...
			case DIALOG_KEY:
				if(data.hasExtra(FilePickerActivity.EXTRA_FILE_PATH)) {
					...
					Log.d(MainActivity.LOGTAG, "Firmware file: " + fileName);
				}
			}
		}
	}
	...
	private void showWaitingDialog(final String message) {
		...
		if(this._isVisible) {
			try {
				...
				ex.printStackTrace();
				Log.e(MainActivity.LOGTAG, "showWaitingDialog: " + ex.toString());
			}
		}
	}
	...
	private void stopWaitDialog() {
		try {
			...
			ex.printStackTrace();
			Log.e(MainActivity.LOGTAG, "stopWaitDialog: " + ex.toString());
		}
	}
	...
	private void handleFirmwareDownloadFinishedResult(final int resultCode) {
		Log.d(MainActivity.LOGTAG, "handleFirmwareDownloadFinishedResult: " + resultCode);
		this.runOnUiThread(new Runnable() {
			@Override
			public void run() {
				try {
	...
	private void handleFirmwareUpdateResult(final int resultCode) {
		Log.d(MainActivity.LOGTAG, "handleFirmwareUpdateResult: " + resultCode);

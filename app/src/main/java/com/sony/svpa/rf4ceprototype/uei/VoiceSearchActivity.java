...
public class VoiceSearchActivity extends RemoteBaseActivity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(MainActivity.HasPermissionToAccessStorage) {
	...
	private void startVoice() {
		try {
			...
			Log.d(MainActivity.LOGTAG, "Start Voice result = " + result);
	...
	private void stopVoice() {
		try {
			...
			Log.d(MainActivity.LOGTAG, "Stop Voice result = " + result);
			if(this._audioTrack != null){
	...
	private IVoiceDataCallback _voiceSearchCallback = new IVoiceDataCallback.Stub() {
		...
		public void onVoiceDataAvailable(int remoteId, byte[] voiceData){
			if(voiceData != null) {
				try {
					// Log.d(MainActivity.LOGTAG, "----- Voice: " + voiceData.length);
		...
		public void onFinished(int result) {
			closeFile();
			try {
				Log.d(MainActivity.LOGTAG, "Voice Search status: " + result + " - " +
	...
	private IKeyEventCallback _BLEKeyEventCallback = new IKeyEventCallback.Stub() {
		...
		public void onKeyEvent(int remoteId, String address, int keyValue) {
			try {
				Log.e(MainActivity.LOGTAG, String.format("----- BLE Key Event: Remote Id %d, Address: %s, KeyCode = %04X",
